...
public class SeatLayout extends JPanel{
	...
	public SeatLayout(Collection<Integer> occupiedSeats, BusBean bb, String date){
		self = this;
		...
		btnBook.addActionListener(new ActionAdapter() {
			...
			for (int i = 1; i <=40 ; i++) { 
				if(seat[i].isSelected()){
					rb = new ReserveBean();
					rb.setPid(Passenger.session.getSessionPassenger().getPid());
					rb.setBusBean(self.bb);
					
					rb.setDate(self.date);
					rb.setSeat(i);
					flag = Passenger.session.reserve(rb);
